...
def evaluate_model(model, test_loader):  # Change parameter name to reflect test_loader
    ...
    write_to_file(f"Accuracy on Dataset: {acc:.2f}%")
...
def write_to_file(text):
    ...
    """
    with open("cifar_res.txt", "a") as file:
        file.write(text + "\n")
...
def main():
    ...
    for i in range(50):
        ...
        model = BaseCNN(num_classes=2).to(device)  # Reinitialize model
        optimizer = optim.Adam(model.parameters(), lr=0.001)
