...
def create_obscured_loader(train_dir, selected_wnids, batch_size=64, num_workers=4, num_images_per_class=75):
    """
    Creates a dataset loader with transformed (obscured) images.
    
    Args:
        train_dir (str): The path to the training data directory.
        selected_wnids (list): A list of selected wnids for the classes you want to use.
        batch_size (int): The batch size for the DataLoader.
        num_workers (int): The number of workers to load the data.
        num_images_per_class (int): The number of random images to select per class.
    
    Returns:
        DataLoader: The DataLoader for the obscured dataset.
    """
    ...
    # Loop through each selected wnid (class)
    for wnid in selected_wnids:
        # Find indices of images belonging to the current class (wnid)
        class_indices = [i for i, (_, label) in enumerate(train_dataset.samples) if train_dataset.classes[label] == wnid]
        
        # Randomly select images from the class
        selected_class_indices = random.sample(class_indices, num_images_per_class)

        # Add selected samples (path, label) to the list
        for idx in selected_class_indices:
            selected_samples.append((idx, wnid_to_class_id[wnid]))

    # Create the ObscuredDataset using the selected samples
    obscured_dataset = ObscuredImageDataset(train_dataset, selected_samples)

    # Create the DataLoader for obscured images
    obscured_loader = DataLoader(obscured_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    
    return obscured_loader
...
def evaluate_model(model, test_loader):
    ...
    with torch.no_grad():
        for images, labels in tqdm(test_loader, desc="Evaluating"):
            ...
            # for i, path in enumerate(test_loader.dataset.selected_samples):
...
if __name__ == "__main__":
    ...
    test_loader=create_obscured_loader(train_dir=TRAIN_DIR, selected_wnids=selected_wnids, batch_size=100)
